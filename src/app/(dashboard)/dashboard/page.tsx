...
export default async function Dashboard() {
  ...
  return (
    <div className="space-y-8">
      {/* Welcome Banner */}
      <div className="bg-gradient-to-r from-slate-900 to-emerald-950 rounded-2xl p-8 text-white relative overflow-hidden shadow-lg border border-emerald-500/10">
        <div className="relative z-10 max-w-xl space-y-3">
          ...
          <div className="pt-2 flex gap-3">
            <Link 
              ...
              className="bg-emerald-500 hover:bg-emerald-600 text-white text-xs font-semibold px-4 py-2.5 rounded-lg transition-colors inline-flex items-center gap-1 shadow-sm"
            >
        ...
        <div className="absolute right-0 bottom-0 w-80 h-80 bg-emerald-500/10 rounded-full blur-3xl -z-0 translate-x-20 translate-y-20"></div>
      </div>
        ...
        <div className="md:col-span-2">
          <Card className="border-slate-100 shadow-sm">
            <CardHeader className="flex flex-row items-center justify-between border-b border-slate-100 pb-4 space-y-0">
              ...
              <Link href="/dashboard/shipments" className="text-xs font-semibold text-emerald-600 hover:text-emerald-700 hover:underline">
                Tout voir
              </Link>
            </CardHeader>
            <CardContent className="pt-6 space-y-6">
              {shipments.map((shipment) => (
                <div key={shipment.id} className="border border-slate-100 rounded-xl p-5 hover:border-emerald-100 hover:shadow-md transition-all">
                  <div className="flex justify-between items-start mb-4">
                    ...
                    <Badge variant={shipment.status === 'livre' ? 'default' : 'secondary'} className="bg-emerald-50 text-emerald-700 border-emerald-100 hover:bg-emerald-50">
                  ...
                  <div className="relative pt-2">
                    <div className="overflow-hidden h-1.5 text-xs flex rounded bg-slate-100 mb-4">
                      <div style={{ width: progressPercent[shipment.status] }} className="shadow-none flex flex-col text-center whitespace-nowrap text-white justify-center bg-emerald-500"></div>
                    </div>
                    <div className="flex justify-between text-[11px] font-semibold text-slate-400">
                      <span className={shipment.status !== 'navire' ? 'text-emerald-600' : 'font-bold'}>🚢 Port</span>
                      <span className={['douane', 'route', 'livre'].includes(shipment.status) ? 'text-emerald-600' : ''}>📑 Douane</span>
                      <span className={['route', 'livre'].includes(shipment.status) ? 'text-emerald-600' : ''}>🚚 Route</span>
                      <span className={shipment.status === 'livre' ? 'text-emerald-600' : ''}>🏁 Bamako</span>
                    </div>
                  </div>
                </div>
              ))}
        ...
        <div className="space-y-6">
          ...
          <Card className="border-slate-100 shadow-sm">
            ...
            <CardContent className="pt-6 space-y-4">
              ...
              <div className="p-4 bg-emerald-50 rounded-xl border border-emerald-200/50 flex gap-3">
                <Shield className="w-5 h-5 text-emerald-600 shrink-0 mt-0.5" />
                <div>
                  <h4 className="font-bold text-emerald-800 text-xs">Acompte à payer</h4>
          ...
          <Card className="border-slate-100 shadow-sm">
            <CardHeader className="flex flex-row items-center justify-between border-b border-slate-100 pb-4 space-y-0">
              ...
              <Link href="/dashboard/documents" className="text-xs font-semibold text-emerald-600 hover:text-emerald-700 hover:underline">
                Ouvrir coffre-fort
              </Link>
            </CardHeader>
              ...
              {documents.slice(0, 2).map((doc) => (
                <div key={doc.id} className="flex items-center justify-between p-3 rounded-lg hover:bg-slate-50 transition-colors">
                  ...
                  <Badge variant={doc.status === 'Verified' ? 'default' : 'outline'} className={
                    doc.status === 'Verified' ? 'bg-emerald-50 text-emerald-700 border-emerald-100 hover:bg-emerald-50' : 'text-slate-500 border-slate-200'
                  }>
